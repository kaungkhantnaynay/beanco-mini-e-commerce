...
The storefront gains a portable support experience without changing the commerce API or weakening its session boundary. The Next.js server must be able to reach the support service, and the support service needs independent database migration, secrets, monitoring, and deployment configuration. A browser refresh intentionally starts a new support conversation.
